...
function AddModal({ open, onClose, onAdd }) {
  const [form, setForm] = useState({ title: "", impact: "", impactType: "profit", impactPeriod: "yearly", urgency: "medium", deadline: "", assignee: "", notes: "" });
  if (!open) return null;
  const handleSubmit = () => {
    if (!form.title) return;
    onAdd({ ...form, impact: Number(form.impact) || 0, status: "todo", isAutoGenerated: false });
    setForm({ title: "", impact: "", impactType: "profit", impactPeriod: "yearly", urgency: "medium", deadline: "", assignee: "", notes: "" });
    onClose();
  };
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 480 }}>
        <div className="modal-head"><h3>アクション追加</h3><button className="modal-x" onClick={onClose}>&times;</button></div>
        <div className="modal-body">
          <div className="form-grid">
            <label className="form-group"><span className="form-label">タイトル</span><input className="form-input" value={form.title} onChange={(e) => set("title", e.target.value)} /></label>
            <label className="form-group"><span className="form-label">インパクト（万円）</span><input className="form-input" type="number" value={form.impact} onChange={(e) => set("impact", e.target.value)} /></label>
            <label className="form-group"><span className="form-label">種類</span>
              <select className="form-input" value={form.impactType} onChange={(e) => set("impactType", e.target.value)}>
                {Object.entries(TYPE_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
              </select>
            </label>
            <label className="form-group"><span className="form-label">期間</span>
              <select className="form-input" value={form.impactPeriod} onChange={(e) => set("impactPeriod", e.target.value)}>
                <option value="monthly">月間</option><option value="yearly">年間</option><option value="onetime">一時</option>
              </select>
            </label>
            <label className="form-group"><span className="form-label">緊急度</span>
            ...
            <label className="form-group"><span className="form-label">期限</span><input className="form-input" type="date" value={form.deadline} onChange={(e) => set("deadline", e.target.value)} /></label>
            <label className="form-group"><span className="form-label">担当者</span><input className="form-input" value={form.assignee} onChange={(e) => set("assignee", e.target.value)} /></label>
            <label className="form-group" style={{ gridColumn: "1/-1" }}><span className="form-label">メモ</span><input className="form-input" value={form.notes} onChange={(e) => set("notes", e.target.value)} /></label>
          </div>
        </div>
        <div className="modal-foot"><button className="btn" onClick={onClose}>キャンセル</button><button className="btn pr" onClick={handleSubmit}>追加</button></div>
      </div>
    </div>
  );
}
...
export default function Actions({ loans, plData, bsData, monthlyPLData, canEdit = true }) {
  ...
  // 手動追加
  const handleAdd = useCallback(async (action) => {
    const saved = await addActionDoc(action);
  ...
  return (
    <div className="page"><div className="g">
      <div className="ph">
        <div><h2>意思決定</h2><p>数字で優先順位をつけ、経営インパクトの大きい順に実行する。</p></div>
        {canEdit && <div className="pa"><button className="btn pr" onClick={() => setModalOpen(true)}>＋ アクション追加</button></div>}
      </div>
      ...
      {canEdit && <AddModal open={modalOpen} onClose={() => setModalOpen(false)} onAdd={handleAdd} />}
    </div></div>
  );
}
