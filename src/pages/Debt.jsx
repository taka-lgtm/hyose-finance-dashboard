...
function filterLoansByPeriod(loans, periodKey, fiscalMonth) {
  ...
  return loans.filter((l) => {
    // 残高がある融資は常に表示（今期・直近N期すべて）
    if (l.balance > 0) return true;

    // 残高ゼロの融資は、対象期間内に活動していたか判定
    ...
    if (l.endDate) {
      ...
      loanEnd = now; // 残高0かつ終了日不明 → 現時点で完済とみなす
    }
...
function BalanceView({ proj, wRate, tMon, loans, bankFilter, onEdit }) {
  ...
  useEffect(() => { totalChart.current?.destroy(); if (!totalChartRef.current || !projTotals.length) return; totalChart.current = new Chart(totalChartRef.current, { type: "line", data: { labels: projLabels, datasets: [{ label: "借入残高 合計", data: projTotals, borderColor: "#22c994", backgroundColor: "rgba(34,201,148,.08)", fill: true, tension: 0.3, borderWidth: 2.5, pointRadius: 4, pointBackgroundColor: "#22c994" }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { ticks: { callback: (v) => Math.round(v / 10000).toLocaleString() + "万", font: chartFont }, grid: chartGrid }, x: { grid: { display: false }, ticks: { font: chartFont } } } } }); return () => totalChart.current?.destroy(); }, [projTotals, projLabels]);
  useEffect(() => { loanChart.current?.destroy(); if (!loanChartRef.current || !projData.length) return; loanChart.current = new Chart(loanChartRef.current, { type: "line", data: { labels: projLabels, datasets: projData.map((l) => { const cs = getCategoryStyle(l.category); const color = cs.borderColor || getBankColor(l.bank); return { label: l.bank + " " + l.name, data: l.balances, borderColor: color, backgroundColor: color + "18", fill: true, tension: 0.3, borderWidth: 1.5, pointRadius: 2, borderDash: cs.borderDash }; }) }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: chartLegend }, scales: { y: { stacked: true, ticks: { callback: (v) => Math.round(v / 10000) + "万", font: chartFont }, grid: chartGrid }, x: { grid: { display: false }, ticks: { font: chartFont } } } } }); return () => loanChart.current?.destroy(); }, [projData, projLabels]);
  useEffect(() => { bankChart.current?.destroy(); if (!bankChartRef.current || !projBankData.length) return; bankChart.current = new Chart(bankChartRef.current, { type: "line", data: { labels: projLabels, datasets: projBankData.map((b) => ({ label: b.bank, data: b.balances, borderColor: getBankColor(b.bank), fill: false, tension: 0.3, borderWidth: 2, pointRadius: 3 })) }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: chartLegend }, scales: { y: { ticks: { callback: (v) => Math.round(v / 10000) + "万", font: chartFont }, grid: chartGrid }, x: { grid: { display: false }, ticks: { font: chartFont } } } } }); return () => bankChart.current?.destroy(); }, [projBankData, projLabels]);
